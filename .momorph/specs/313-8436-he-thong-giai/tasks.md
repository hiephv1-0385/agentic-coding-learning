...
- [x] T018 [US1] Rewrite awards page.tsx — Server Component: Supabase fetch award_categories (all 14 columns) ordered by "order" ASC, snake_case→camelCase mapping, empty state handling ("Chưa có thông tin giải thưởng."). Full composition: Header(activeLink="awards"), AwardsHeroSection, main(max-w-[1440px] mx-auto negative-mt overlap, flex-col gap-section-gap) with SectionTitle, award system section (flex-col lg:flex-row gap-awards-gap: AwardSidebar + cards column gap-section-gap), AwardsSunKudosSection, Footer per plan Phase 1.6 | src/app/awards/page.tsx
